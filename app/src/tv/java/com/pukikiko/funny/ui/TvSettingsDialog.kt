...
/**
 * The handful of settings that make sense on a TV: which instance to watch,
 * how the feed is picked, and playback volume. Sharing, downloading and
 * uploading live on the mobile flavour, where there is somewhere to put them.
 */
@OptIn(ExperimentalTvMaterial3Api::class)
@Composable
fun TvSettingsDialog(feed: FeedController, onDismiss: () -> Unit) {
    ...
    Box(
        ...
        Column(
            ...
            Spacer(Modifier.height(8.dp))
            ProgressBar(progress = feed.volume, modifier = Modifier.width(320.dp), height = 6.dp)
            Spacer(Modifier.height(8.dp))
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
